...
type ProgramTaskRow = ProgramTask & { category?: ProgramCategory };

export default function ManageProgramTasks({
  programId,
  tasks,
  categories,
}: {
  programId: string;
  tasks: ProgramTaskRow[];
  ...
  const [selectedTask, setSelectedTask] = useState<ProgramTaskRow | null>(null);
  ...
  const openEdit = (task: ProgramTaskRow) => {
  ...
  const handleDeleteTask = async () => {
    ...
    try {
      const response = await fetch(
        `/api/programs/${programId}/tasks?task_id=${selectedTask.id}`,
        { method: "DELETE" },
      );
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Failed");
  ...
  const scheduleBadge = (task: ProgramTaskRow) => {
    if (task.schedule_type === "dated") {
      return `${task.start_date} → ${task.end_date}`;
    }
    const days = task.days ?? [];
    if (days.length === ALL_DAYS.length) return "يوميا";
    return days
        ...
        >
          <div className="flex flex-col items-start">
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              ...
              <span className="ds-badge-primary">{scheduleBadge(task)}</span>
            </div>
          </div>
        </button>
      ))}
