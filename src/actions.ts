...
import { supabaseAdmin, supabaseNew } from "./lib/supabase";
import { getUserProgramTasksSections } from "./lib/programs";

export {
  getFriendIdsInProgram,
  getProgramById,
...
export const getSessionUser = async (): Promise<User | null> => {
  try {
    ...
    const { data: connection, error } = await supabaseNew
      .from("connections")
      .select("*")
...
export const getUsers = async () => {
  try {
    const { data: users, error } = await supabaseNew
      .from("users")
      .select("*, connections(*)")
