...
export async function PUT(
  ...
  if (is_done === undefined) {
    return NextResponse.json({ error: 'is_done field is required' }, { status: 400 });
  }
  ...
  if (userTask.user_id !== session.user_id && !['admin', 'owner'].includes(user.role)) {
    return NextResponse.json({ error: 'Forbidden - Can only update your own tasks' }, { status: 403 });
  }
